...
from .swin_transformer_v2_new import DualSwinTransformer
...
def build_model(config, is_pretrain=False):
    ...
    if model_type == 'swin':
        ...
        model = DualSwinTransformer(img_size=config.DATA.IMG_SIZE,
                                  patch_size=config.MODEL.SWINV2.PATCH_SIZE,
                                  in_chans=config.MODEL.SWINV2.IN_CHANS,
                                  num_classes=config.MODEL.NUM_CLASSES,
                                  embed_dim=config.MODEL.SWINV2.EMBED_DIM,
                                  ...
                                  head_drop_rate=config.MODEL.HEAD_DROP_RATE,
                                  dynamic=config.MODEL.DYNAMIC,
                                  ca_flag=config.MODEL.CA_FLAG)
